...
1. ON THE HOST: Run `vagrant up` in this folder (where the README is located) and then login into the machine:
   ```
   $ vagrant up
   $ vagrant ssh
   ```
...
4. ON THE GUEST VM: If you want to restart the server and run it again:
   ```
   $ source ~/.bashrc
   $ workon b2share
