...
from invenio.simpledeposit_load_deposition_types import deposition_metadata
...
def delete_workflow(user_id, deposition_type, uuid):
    """ Deletes all workflow related data
        (workflow and drafts)
    """
    db.session.query(WebDepositDraft).filter_by(uuid=uuid,
                                                user_id=user_id).delete()
    db.session.query(WebDepositWorkflow).filter_by(uuid=uuid).delete()
    db.session.commit()
