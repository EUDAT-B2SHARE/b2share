...
the host machine (by default  ``/b2share-data`` ) into the b2share container.
This folder is used to store all the data uploaded into B2SHARE and is
therefore very important; you can change its location or name by editing the
``dockerize/docker-compose.yml`` file (see the ``services > b2share >
volumes`` section). If you are only trying to run a B2SHARE demo and you are
certain that the data put in your B2SHARE installation doesn't need to be
preserved, you can erase the entire ``volumes`` section from the
...
(which also by default is mounted to the host folder ``/b2share-data/`` as
described above). The name of this configuration file is ``b2share.cfg``.
Please note that after each update of this file the service needs to be
restarted (with e.g.``docker-compose restart``) for the changes to take
effect.

By default, the B2SHARE service will use an internal sqlite database. This
configuration can only be used for demonstration purposes, as the database
will be lost on each recreation of the Docker container. An external database
can be configured by editing the ``b2share.cfg`` file and defining the
...
browsers. For a production server it is recommended to use a custom generated
