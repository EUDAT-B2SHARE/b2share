...
from b2share.modules.management.ownership.utils import find_version_master
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            if creator is not None:
                login_user(creator, client)
            url = url_for('b2share_ownership.record_ownership',
                          record_pid=pid.pid_value, email=new_owner.email, _external=True)
            url = parse.unquote(url)
            headers = [('Accept', '*/*'), ('Content-Length', '0'),
                       ('Accept-Encoding', 'gzip, deflate, br')]
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            if creator is not None:
                login_user(creator, client)
            url = url_for('b2share_ownership.record_ownership',
                          record_pid=fake_pid, email=new_owner.email, _external=True)
            headers = [('Accept', '*/*'), ('Content-Length', '0'),
                       ('Accept-Encoding', 'gzip, deflate, br')]
            url = parse.unquote(url)
            res = client.put(url, headers=headers)
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            if creator is not None:
                login_user(creator, client)
            url = url_for('b2share_ownership.record_ownership',
                          record_pid=pid.pid_value, email=fake_email, _external=True)
            headers = [('Accept', '*/*'), ('Content-Length', '0'),
                       ('Accept-Encoding', 'gzip, deflate, br')]
            url = parse.unquote(url)
            res = client.put(url, headers=headers)
...
def test_ownership_with_preset(app, login_user, test_records_data):
    with app.app_context():
        creator = create_user('creator')
        app.config['DEFAULT_OWNERSHIP'] = [creator.email]
        new_owner = create_user('new_owner')
        _deposit, pid, _record = create_record(test_records_data[0], new_owner)

        with app.test_client() as client:
            if creator is not None:
                login_user(creator, client)
            url = url_for('b2share_ownership.record_ownership',
                          record_pid=pid.pid_value, _external=True)
            headers = [('Accept', '*/*'), ('Content-Length', '0'),
                       ('Accept-Encoding', 'gzip, deflate, br')]
            url = parse.unquote(url)
            res = client.get(url, headers=headers)
            ...
            found = False
            for x in data:
                if x['email'] == creator.email:
                    if x['preset']:
                        found = True
            assert found

def test_ownership_delete(app, login_user, test_records_data):
    with app.app_context():
        creator = create_user('creator')
        app.config['DEFAULT_OWNERSHIP'] = [creator.email]
        new_owner = create_user('new_owner')
...
def test_ownership_delete_no_owners_left(app, login_user, test_records_data):
    with app.app_context():
        creator = create_user('creator')
        _deposit, pid, _record = create_record(test_records_data[0], creator)
        ...
        with app.test_client() as client:
            if creator is not None:
                login_user(creator, client)
            url = url_for('b2share_ownership.record_ownership',
                          record_pid=pid.pid_value, email=creator.email, _external=True)
            headers = [('Accept', '*/*'), ('Content-Length', '0'),
                       ('Accept-Encoding', 'gzip, deflate, br')]
            url = parse.unquote(url)
            res = client.delete(url, headers=headers)
...
def test_ownership_delete_not_found_owner(app, login_user, test_records_data):
    with app.app_context():
        creator = create_user('creator')
        _deposit, pid, _record = create_record(test_records_data[0], creator)
        new_owner = create_user('new_owner')
