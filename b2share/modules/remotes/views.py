...
class Jobs(ContentNegotiatedMethodView):
    ...
    def post(self):
        ...
        fileBucket_urlbase = url_for('invenio_files_rest.bucket_collection_api', _external=True)
        ...
        if destination_file_url.endswith('/'):
            destination_file_url = destination_file_url[:-1]
        pieces = destination_file_url.split('/')
        key = pieces[-1]
        bucket_id = pieces[-2]
        if not key or not bucket_id:
            raise UserError("bad destination_file_url; must be a correct URL pointing to a file bucket object")
...
def put_file_into_bucket(bucket_id, key, stream, content_length):
    ...
    # WARNING: this function should be a copy of invenio_files_rest.views:ObjectResource.put
    ...
    try:
        ...
        obj.set_contents(stream, size=content_length)
        db.session.commit()
        return {'json': {
